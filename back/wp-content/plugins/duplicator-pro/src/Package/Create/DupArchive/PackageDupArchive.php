    ...
    public static function create(DUP_PRO_Package $package, BuildProgress $buildProgress)
    {
        try {
            ...
            if ($buildProgress->retries > DUP_PRO_Constants::MAX_BUILD_RETRIES) {
                $error_msg = __('Package build appears stuck so marking package as failed. Is the Max Worker Time set too high?.', 'duplicator-pro');
            ...
            try {
                ...
                $fix    = sprintf(
                    _x(
                        'Package build failure due to building a large package on 32 bit PHP. Please see %1$sTech docs%2$s for instructions on how to resolve.',
