...
export default function Navbar({ translations, locale }) {
    ...
    return (
        <header className='sticky top-0 z-40 flex h-14 justify-center bg-[#ffffffe0] shadow' style={{ backdropFilter: 'blur(4px)' }}>
            <nav className='m-3 flex w-full'>
                <div className='container mr-5 flex w-full max-w-full items-center justify-between'>
                    {/* Logo section */}
                    <div className='flex items-center'>
                        <div className='navLogo mx-2 h-[20px] w-[120px] md:h-[23px] md:w-[143px]'>
                    ...
                    {/* Desktop Links */}
                    <div className='mx-2 flex items-center gap-x-6 max-lg:hidden xl:gap-x-10'>
                        {translations?.liens.map((item, index) => {
                            if (!item.url || !item.link) return null;

                            const resolvedUrl = item.url.replace('${locale}', locale);
                            ...
                            return (
                                <Link
                                    ...
                                    className='text-xs transition hover:text-green-500 lg:text-sm'
                                >
                                    {item.link}
                                </Link>
                            );
                    ...
                    {/* Liens applis */}
                    {!isClick && (
                        <div className='flex max-h-full max-md:hidden'>
                            <div className='flex flex-row items-center lg:mx-1 lg:gap-0.5'>
                                ...
                                >
                                    <Image
                                        src={
                                            translations['Footer']?.AppStoreImg ||
                                            '/images/General/Download_on_the_App_Store_Badge_FR_blk_100517.webp'
                                        }
                                        alt=''
                                        width={120}
                                        height={120}
                    ...
                    {/* Mobile Menu Button */}
                    <div className='mobile-menu hidden items-center max-lg:flex'>
                        <button
                            aria-label='toggle button'
                            aria-expanded='false'
                            id='menu-btn'
                            className='btn-menu inline-flex cursor-pointer items-center justify-center rounded-md p-4 focus:outline-none'
                        ...
                        >
                            {isClick ? (
                                <Image
                                    src={Croix}
                                    alt=''
                                    className='absolute right-4 top-4 mr-4 h-6 w-6'
                                    width={32}
                                    height={32}
                                    priority={true}
                                    //layout="responsive"
                    ...
                    {/* Mobile Menu Links */}
                    {isClick && (
                        <div className='absolute left-0 right-0 top-[56px] flex w-auto max-w-full flex-col justify-center bg-white shadow-lg lg:hidden'>
                            <div className='space-y-1 px-4 pb-3 text-center sm:px-6'>
                                {translations?.liens.map((item, index) => {
                                    if (!item.url || !item.link) return null;
                                    ...
                                    return (
                                        <Link
                                            ...
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='lien-mobile block break-words p-2 text-sm md:text-base'
                                        >
                                            {item.link}
                                        </Link>
                                    );
