...
export default function Bloc1({ translations, isArabic }) {
    return (
        <div
            className={`kal-hero min-h-[500px] bg-white bg-hero-pattern bg-cover object-cover px-12 py-20 ${isArabic && 'kal-hero-arabic'}`}
        >
            <div>
                <h1>{translations['titre_bloc1']}</h1>

                <section>
                    {translations['repeteurCheckGreenBloc1']?.map((item, index) => (
                        <div key={index}>
                    ...
                    <p className={styles.bloc1Partie1H4}>
                        {translations['titre_h4_bloc1'].split('\\esc').map((part, index) => (
                            <React.Fragment key={index}>
                                {part}
                                {index < translations['titre_h4_bloc1'].split('\\esc').length - 1 && <br />}
                            </React.Fragment>
                        ))}
                    </p>
                </section>
                ...
                <aside>
                    {translations['repeteurCheckWhiteBloc1']?.map((item, index) => (
                        <div key={index}>
                ...
                <article className='absolute bottom-0 -mb-20 -mt-20 flex h-[600px] w-[497px] items-end justify-center lg:right-[calc(-5%)]'>
                    <Image src={translations['girl']} alt='Hero Girl' width={497} height={600} priority={true} className='object-contain' />
                </article>
            </div>
        </div>
    );
