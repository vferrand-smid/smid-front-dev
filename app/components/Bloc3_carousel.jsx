...
const Carousel = ({ page, locale }) => {
    ...
    useEffect(() => {
        if (translations?.Bloc3?.repeteur_etape?.length) {
            const interval = setInterval(() => {
                setCurrentDesktop((prevCurrent) => (prevCurrent + 1) % translations.Bloc3.repeteur_etape.length);
            }, 1500);
    ...
    useEffect(() => {
        if (translations?.Bloc3?.repeteurEtapeMobile?.length) {
            const interval = setInterval(() => {
                setCurrentMobile((prevCurrent) => (prevCurrent + 1) % translations.Bloc3.repeteurEtapeMobile.length);
            }, 1500);
            return () => clearInterval(interval);
        }
    }, [translations]);
    ...
    const handleArrowClickMobile = (direction) => {
        setCurrentDesktop((prevCurrent) =>
            direction === "left"
                ? (prevCurrent - 1 + translations.Bloc3.repeteurEtapeMobile.length) % translations.Bloc3.repeteurEtapeMobile.length
                : (prevCurrent + 1) % translations.Bloc3.repeteurEtapeMobile.length
        );
    };
                            ...
                            <div>
                                {translations?.Bloc3?.repeteur_etape?.map((_, index) => (
                                    <section
                                        ...
                                        style={{ backgroundColor: index === (window.innerWidth < 640 ? currentMobile : currentDesktop) ? 'green' : 'lightgrey' }}
                                    />
                                ))}
                            </div>
