...
const KalSearch = ({ locale }) => {
	const [currentCountry, setCurrentCountry] = useState(null);  // Pays détecté via l'IP
	const [selectedCountry, setSelectedCountry] = useState(null);  // Pays sélectionné par l'utilisateur
	const [selectedDocument, setSelectedDocument] = useState(null);
	const [countries, setCountries] = useState([]);
	const [documents, setDocuments] = useState([]);
	const [isCountryPopupVisible, setIsCountryPopupVisible] = useState(false);
	...
	useEffect(() => {
		const fetchUserCountry = async () => {
			try {
				const response = await fetch('https://ipapi.co/country/');
				const countryCode = await response.text();
				console.log('User country detected:', countryCode);  // Log pour vérifier si le bon pays est détecté
				setCurrentCountry(countryCode);
			} catch (error) {
				console.error('Error fetching user country:', error);
			}
	...
	useEffect(() => {
		if (currentCountry && selectedCountry && selectedDocument) {
			setIsReady(true);
		}
	}, [currentCountry, selectedCountry, selectedDocument]);

	// Redirection sans exposer le pays d'origine dans l'URL
	const handleRedirection = () => {
		if (isReady) {
			...
			const url = `https://smartphone-id-app.com/${platform}/photo/${selectedDocument}/${selectedCountry}/${languageCode}`;
			console.log('Redirecting to:', url, 'from current country:', currentCountry);

			window.open(url, '_blank');
		}
	};

	// Fetch des pays côté client uniquement
	useEffect(() => {
		const fetchCountries = async () => {
			try {
				const response = await fetch('https://smartphoneid-api--master-2yx5ebbula-ew.a.run.app/country/customized');
		...
		fetchCountries();  // Appel API côté client uniquement
	}, []);

	// Fetch des documents pour un pays sélectionné (côté client uniquement)
	useEffect(() => {
		const fetchDocuments = async () => {
			if (selectedCountry) {
				try {
					console.log('Fetching documents for selected country:', selectedCountry);
					...
					if (response.ok) {
						const data = await response.json();
						console.log('Documents fetched for current country:', currentCountry, 'and selected country:', selectedCountry);
		...
		fetchDocuments();  // Appel API côté client uniquement
	}, [selectedCountry]);
	...
	return (
		<div className="kal-search md:flex-row md:items-center md:justify-between p-4 md:max-sm:w-[400px] md:w-[750px] md:h-[115px] rounded-lg border-2 border-[#efefef] bg-white ">
			<div className="md:max-sm:grid md:max-sm:grid-rows-3 md:max-sm:justify-items-start md:max-sm:gap-4 md:flex md:flex-row w-full">
				{/!* COUNTRY *!/}
				<div className="kal-search-country md:max-sm:flex w-full sm:w-auto border-b-2 md:border-r-2 border-b-gray-100 md:border-r-gray-100 relative pr-5">
					<div>
						<h4 className="text-left md:mt-4 md:ml-3 text-black font-['Georama'] text-[.9375rem] font-semibold leading-[normal]">
							...
							<svg width="13" height="8" viewBox="0 0 13 8" fill="none" xmlns="http://www.w3.org/2000/svg" className="inline ml-2 mr-4">
								<path fillRule="evenodd" clipRule="evenodd" d="M12.6192 0.365305C12.1116 -0.121769 11.2884 -0.121769 10.7808 0.365305L6.5 4.4723L2.21924 0.365304C1.71156 -0.121769 0.888443 -0.12177 0.380762 0.365304C-0.126921 0.852378 -0.126921 1.64208 0.380762 2.12915L5.80769 7.33579C6.19459 7.70699 6.8054 7.70699 7.19231 7.33579L12.6192 2.12916C13.1269 1.64208 13.1269 0.852379 12.6192 0.365305Z" fill="#2FC977"/>
							</svg>
						</h4>
					</div>

					<section className="flex gap-4 items-center p-4 cursor-pointer" onClick={() => setIsCountryPopupVisible(!isCountryPopupVisible)}>
						{selectedCountry && (
							<>
								<Image width={500} height={500} src={`https://purecatamphetamine.github.io/country-flag-icons/3x2/${selectedCountry}.svg`} alt="country flag" className="w-10 h-10" />
								<span className="text-base">{countries.find(c => c.code === selectedCountry)?.name}</span>
							</>
						)}
					</section>
					...
					{isCountryPopupVisible && (
						<main className="kal-search-country-popup absolute shadow-lg rounded-lg p-2 mt-2 mr-4 z-10 bg-white">
							<div className="kal-search-country-search-container mb-4">
								<input type="text" placeholder="Rechercher un autre pays..." className="w-full p-2 border border-gray-300 rounded-md" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
							</div>
							<div className="kal-search-country-search-suggestion overflow-y-auto grid grid-cols-2 gap-2 p-2">
								{filteredCountries.length > 0 ? (
									filteredCountries.map(country => (
										<div key={country.code} className={`flex items-center p-2 border ${selectedCountry === country.code ? 'border-green-500' : 'border-gray-300'}`} onClick={() => handleCountrySelect(country)}>
											<img src={country.flag} alt={country.name} className="w-8 h-8"/>
											<span className="ml-2">{country.name}</span>
										</div>
									))
				...
				<div className="kal-search-document relative pr-5">
					<div>
						<h4 className="text-left mt-4 text-black font-['Georama'] text-[.9375rem] font-semibold leading-[normal]">
							{translations.kalSearch['titre_2']} <span className="text-red-500">*</span>
							<svg width="13" height="8" viewBox="0 0 13 8" fill="none" xmlns="http://www.w3.org/2000/svg" className="inline ml-2">
								<path fillRule="evenodd" clipRule="evenodd" d="M12.6192 0.365305C12.1116 -0.121769 11.2884 -0.121769 10.7808 0.365305L6.5 4.4723L2.21924 0.365304C1.71156 -0.121769 0.888443 -0.12177 0.380762 0.365304C-0.126921 0.852378 -0.126921 1.64208 0.380762 2.12915L5.80769 7.33579C6.19459 7.70699 6.8054 7.70699 7.19231 7.33579L12.6192 2.12916C13.1269 1.64208 13.1269 0.852379 12.6192 0.365305Z" fill="#2FC977"/>
							</svg>
						</h4>
					</div>
					...
					<section className="flex gap-4 items-center p-4 cursor-pointer" onClick={() => setIsDocumentPopupVisible(!isDocumentPopupVisible)}>
						{selectedDocument && (
							<>
								<Image width={500} height={500} src={documents.find(d => d.id === selectedDocument)?.img} alt="document icon" className="w-10 h-10" />
								<span className="text-base">{documents.find(d => d.id === selectedDocument)?.name}</span>
							</>
						)}
					</section>
					...
					{isDocumentPopupVisible && (
						<main className="kal-search-document-popup absolute shadow-lg rounded-lg p-2 mt-2 mr-4 z-10 bg-white">
							<div className="kal-search-document-search-suggestion overflow-y-auto grid grid-cols-2 gap-2 p-2">
								{documents.map(doc => (
									<div key={doc.id} className={`flex flex-col items-center p-2 border ${selectedDocument === doc.id ? 'border-green-500' : 'border-gray-300'}`} onClick={() => handleDocumentSelect(doc)}>
										<Image width={500} height={500} src={doc.img} alt={doc.name} className="w-10 h-10 object-contain"/>
										<span className="ml-2">{doc.name}</span>
									</div>
								))}
							</div>
				...
				{/!* BOUTON *!/}
				<button className={`flex-shrink-0 h-[3.25rem] self-center rounded-[26px] bg-black text-white w-[148px] mx-auto ${isButtonDisabled ? 'opacity-60 cursor-not-allowed' : ''}`} onClick={handleRedirection} disabled={!isReady}>
					{translations.kalSearch['bouton']}
				</button>
